...
class PriceIntegrator:
    """Integrates market price data from multiple sources."""
    ...
    def _get_yahoo_price(self, ticker: str) -> Optional[Dict[str, Any]]:
        """Fetch price data from Yahoo Finance with rate limit handling."""
        try:
            ...
            if '429' in error_msg or 'Too Many Requests' in error_msg:
                logger.warning(f"⚠️  Yahoo Finance rate limit hit for {ticker} - will use fallback")
            else:
                logger.error(f"Error fetching Yahoo price for {ticker}: {e}")
            return None
    ...
    def get_batch_prices(self, tickers: List[str], source: str = 'yahoo') -> Dict[str, Dict[str, Any]]:
        """Fetch prices for multiple tickers with rate limit protection."""
        prices = {}
